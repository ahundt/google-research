...
class MBConvDecoder(object):
  ...
  def _decode_block_string(self, block_string):
    ...
    return singlepath_supernet.BlockArgs(
        ...
        dilations=[int(options['d'][0]), int(options['d'][1])])
  ...
  def _encode_block_string(self, block):
    ...
    args = [
        ...
        'd%d%d' % (block.dilations[0], block.dilations[1]) # todo make changes for dilation
    ]
...
def build_supernet(images, model_name, training, prefix='' override_params=None, dropout_rate=None):
  ...
  """
  assert isinstance(images, tf.Tensor) # check if the image is of type tf.Tensor
