...
class MBConvBlock(object):
  ...
  def _build(self):
    ...
    # underscore prefix is meant as a hint to another programmer that a variable
    ...
    if self._search_space is None: #  for "default" layers
      ...
      self._depthwise_conv = tf.keras.layers.DepthwiseConv2D(
        ...
        dilation_rate=self._block_args.dilations, # https://www.tensorflow.org/api_docs/python/tf/keras/layers/DepthwiseConv2D?hl=es-419
    ...
    # Learnable Depth-wise convolution Superkernel
      ...
      self._depthwise_conv = DepthwiseConv2DMasked(
        [kernel_size, kernel_size],
        strides=self._block_args.strides, 
        dilation_rate=self._block_args.dilations,
        # todo make changes for dilation
        depthwise_initializer=conv_kernel_initializer,
  ...
  def _call_se(self, input_tensor):
    ...
    # todo change the function to swish https://www.tensorflow.org/api_docs/python/tf/keras/activations/swish#:~:text=Swish%20activation%20function%20which%20returns,unbounded%20above%20and%20bounded%20below. 
...
class SinglePathSuperNet(tf.keras.Model):
  ...
  def __init__(self, blocks_args=None, global_params=None,dropout_rate=None):
    ...
    with open(lutmodel_filename, 'r') as f:
      self._runtime_lut = json.load(f) # load the data from json
  ...
  def call(self, inputs, training=True):
    ...
    call method enables Python programmers to write classes where the instances behave like functions and can be called like a function
    Args:
